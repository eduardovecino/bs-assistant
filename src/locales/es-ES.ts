...
export const SPANISH_TRANSLATIONS = {
    ...
    'intent.account.selected_account_%account%': `Has seleccionado la cuenta {{ account }}. ¿Qué quieres hacer a continuación?`,
    'intent.account.selected_account.failure_%account%': `No podemos mostrar la cuenta terminada en {{ account }}.`,
    'intent.account.simple_response.screen_%number%': `Tienes {{ number }} cuentas. ¿Cúal deseas seleccionar?`,
    'intent.account.simple_response.no_screen_%number%_%accounts%': `Tienes {{ number }} cuentas. Terminadas en: {{ accounts }} ¿Cúal deseas seleccionar?`,
    ...
    'intent.card.simple_response.screen_%number%': `Tienes {{ number }} tarjetas. ¿Cúal deseas seleccionar?`,
    'intent.card.simple_response.no_screen_%number%_%cards%': `Tienes {{ number }} tarjetas. Terminadas en: {{ cards }} ¿Cúal deseas seleccionar?`,
    'intent.card.selected_card_%card%': `Has seleccionado la tarjeta finalizada en {{ card }}. ¿Qué quieres hacer a continuación?`,
    ...
    'intent.information.contact.simple_response': `Puedes contactarnos, cualquier día y a cualquier hora, a través del teléfono: ${Ssml.prosody(Ssml.sayAs({ text: "902 323 000", interpret: As.characters }), { rate: "x-slow"})} `,
}
