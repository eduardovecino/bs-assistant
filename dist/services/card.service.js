...
class CardService extends rest_manager_1.RestManager {
    getCards() {
        return new Promise((resolve, reject) => {
            ...
            const cards = [];
            data.forEach(card => cards.push(new card_model_1.CardModel(card)));
    ...
    getCard(last4) {
        return new Promise((resolve, reject) => {
            const data = fs.readFileSync('mock/card/get-cards.json');
            // const jsonData = JSON.parse(data.toString());
            const card = new card_model_1.CardModel(card_manager_1.CardManager.getCardByLast4(data, last4));
